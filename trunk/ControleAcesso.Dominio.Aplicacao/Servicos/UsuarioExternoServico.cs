...
namespace ControleAcesso.Dominio.Aplicacao.Servicos
{
    public class UsuarioExternoServico : Servico<UsuarioExterno>
    {
        ...
        public virtual UsuarioExterno Autenticar(string codigoSistema, string login, string senha)
        {
           // var lista = Instancia.Buscar(x => x.Login.ToUpper().Equals(login.ToUpper()));
            
           var lista = Instancia.Buscar(u => u.Login.ToUpperInvariant().Equals(login.ToUpperInvariant().Trim()));
            //var lista = UsuarioExternoServico.Instancia.Buscar();
            if (lista.Any())
            {
        ...
        {
            var usuario = this.Buscar(u => u.Login.ToUpperInvariant().Equals(login.ToUpperInvariant().Trim())).FirstOrDefault();
            if (usuario != null)
            {
        ...
        private void DesativarSenhaPermanente( string login)
        {
            ...
            {
                var servico = Servico<UsuarioExternoSenha>.Instancia.Buscar(p =>
                     p.Login.Trim().ToLower().Equals(login.Trim().ToLower()) && p.Tipo.Equals("P")).FirstOrDefault();

                if (servico != null)
                {
                    Servico<UsuarioExternoSenha>.Instancia.Excluir(servico);
                }
            }
        }
