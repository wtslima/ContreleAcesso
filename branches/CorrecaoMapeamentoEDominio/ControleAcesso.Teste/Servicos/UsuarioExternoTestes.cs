...
namespace ControleAcesso.Teste.Servicos
{
    ...
    public class UsuarioExternoTestes
    {
        ...
        [TestCase(1, "dcbelmont@inmetro.gov.br", "123mudarei")]
        [TestCase(2, "dcbelmont@inmetro.gov.br", "", ExpectedException = typeof(SenhaInvalidaException))]
        [TestCase(3, "dcbelmont@gmail.com", "", ExpectedException = typeof(LoginInexistenteException))]
        [TestCase(3, "dcbelmont@inmetro.gov.br", "123mudarei", ExpectedException = typeof(AcessoNegadoException))]
        [TestCase(1, "dcbelmont@inmetro.gov.br", "senhaTemporaria", ExpectedException = typeof(SenhaTemporariaException))]
        public void Autenticar(int sistema, string login, string senha)
        {
        ...
        public void AutenticarSenhaTemporariaExpirada() {
          ...
          //  usuario.SenhaTemporaria.Expiracao = DateTime.Now.AddDays(-14);
        	...
        	servicoUsuario.Autenticar(1, login, Criptografar(senhaTemporaria));
        }
        ...
        public void SolicitarSenhaTemporaria() {
           ...
           var senha = _servicoSenha
                .Buscar(s => s.UsuarioExterno.Login.ToLower().Equals(login.ToLower()) && s.Tipo.Equals("T") && s.Excluido == false && s.Expiracao.CompareTo(DateTime.Now) > 0)
                .FirstOrDefault();
            Assert.IsNotNull(senha);
           // Assert.IsTrue(servico.ValidarSenha(senha.UsuarioExterno, Criptografar(senhaTemporaria)).IsTemporaria);
            Assert.AreEqual(login.ToLower(), senha.UsuarioExterno.Login.ToLower());
        }
        ...
        public void CriarUsuarioExistente(string login, int idPessoaFisica)
        {
            ...
            usuario.AdicionarPerfil(new SistemaPerfil { CodigoSistema = 1, CodigoPerfil = 5 });
