    ...
    public class AutenticacaoServico : IAutenticacaoServico
    {
        ...
        public AutenticacaoServico()
        {
            ...
            Mapper.CreateMap<Entidades.Usuario, Usuario>()
                ...
                .ForMember(dst => dst.ControleAtivo, src => src.MapFrom(p => p.Perfis.Count() > 0 ? "A" : "D"));
            Mapper.CreateMap<Entidades.UsuarioExterno, Usuario>()
                ...
                .ForMember(dst => dst.ControleAtivo, m => m.MapFrom(src => src.Perfis.Count() > 0 ? "A" : "D"));
        ...
        public void AtualizarPerfis(string token, string login, List<string> perfis)
        {
            ...
            try
            {
                if (SistemaServico.Instancia.ValidarToken(token, UserHostName))
                {
                    var codigoSistema = SistemaServico.Instancia.DecomporToken(token, UserHostName).CodigoSistema;
        ...
        public Usuario AutenticarUsuarioExterno(string token, Login login)
        {
            ...
            try
            {
                if (SistemaServico.Instancia.ValidarToken(token, UserHostName))
                {
                    origem = SistemaServico.Instancia.DecomporToken(token, UserHostName);
        ...
        private void EnviarEmail(string para, string assunto, string urlTemplate, EmailData modelo)
        {
            try
            {
                ...
                client.Credentials = new System.Net.NetworkCredential("controleacesso-naoresponda", "$s9KGrM7Km");
