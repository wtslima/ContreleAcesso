    ...
    public class UsuarioExternoServico : Servico<UsuarioExterno>
    {
        ...
        public UsuarioExterno Autenticar(string codigoSistema, string login, string senha)
        {
            ...
            if (lista.Count() > 0)
            {
                ...
                #region Tratando perfis do usuário
                var perfisInativos = usuario.Perfis.Where(p => !p.Ativo || !p.Ativo || !p.Perfil.Ativo).ToList();
                perfisInativos.ForEach(p => usuario.RemoverPerfil(p));
                var perfisOutrosSistemas = usuario.Perfis.Where(p => !p.CodigoSistema.ToUpper().Equals(codigoSistema.ToUpper())).ToList();
        ...
        public UsuarioExterno CriarUsuario(string login, int pessoaId, string nome, string codigoSistema, string codigoPerfil)
        {
            ...
            var usuarioExternoCadastrado = usuarioExternoServico.Buscar(us => us.IdPessoaFisica.Equals(pessoaId) && us.Ativo == true);
